...
- `src/slidesdemo.js` — the `SLIDES_DEMO` "Slides 101" example deck constant (seeded on first run + "🎞 Add example slideshow" button)
...
- **`onInlineInput` mirrors `innerHTML`, not `textContent`** — editing the text around `<b>`/`<a>` children keeps those children intact (textContent would have flattened them). Child `data-hs-id`s ride along in the mirror and stay consistent (display is a clone of the annotated srcDoc); serialization strips them as always.
- Inspector hint for non-leaf selections now says "double-click any text on the page to edit it in place".

**Added in v1.15 (July 9 2026) — slideshow upgrades + Slides 101 example:**
- **Present controls** (`PRESENT_JS` rewrite, still string-concat/no `</script` literal): click/tap anywhere advances (guarded — clicks on `a,button,input,select,textarea,video,audio,[onclick]` are never stolen), on-screen ‹ › circle arrows (fade with the HUD after 3s, wake on mousemove), 4px cyan progress bar along the bottom, **B** toggles a blackout div (created LAST at max z-index so it covers the controls; click or Esc un-blanks — Esc only exits when not blacked out), typing digits + 650ms pause jumps to that slide (HUD echoes "→ n"), Home/End.
- **Slide layouts**: every add-slide path (filmstrip header ＋ Add, per-thumb ＋, Insert → Slide) opens a `#layoutMenu` picker — `SLIDE_LAYOUTS` in slides.js: Title / Bullets / Two columns / Quote / Section divider / Blank. Layouts are section INNER html only (plain h2/p/ul so the deck's own CSS styles them; inline flex only for the two-column shell); the base-class intersection rule still applies.
- **Slide numbers & footer** (⚙ in the filmstrip header): writes a managed `<style data-hs-slidedeco>` into the file's `<head>` — pure CSS counters (`section::after` = "n / total", `section::before` = footer text, absolute-positioned into the slide corners), so it renders everywhere the file goes. The style block IS the state: `parseDeco` prefills the dialog from it; both off removes the block. `withEdit` calls `syncDecoCount()` after every structural op so the baked "/ total" stays honest. Footer text is CSS-string-escaped both ways.
- **Slides 101 example deck** (`src/slidesdemo.js`): 7-slide guided playground (filmstrip ops, layouts, ⚙, Present keys, "still just HTML", make-your-own), tagged `slideshow`+`guide` AND `<body data-hs-slideshow>`, ships with the deco block ON (format matches `decoCSS` exactly so ⚙ parses it). Seeded on first run BEFORE Welcome (shelf sorts by modified — Welcome stays first), re-addable via sidebar "🎞 Add example slideshow" (`#btnSlides101`).
